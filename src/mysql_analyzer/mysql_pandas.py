...
class mysql_profiler:
    ...
    def multiple_dataset_apply_mysql(self):
        ...
        pattern_d_alt = re.compile(
            r"[0-9]{2}.[0-9]{2}.[0-9]{4}.*", re.IGNORECASE)

        # Creating connection string
        db = mysql.connector.connect(host=str(self.host), user=str(
        ...
        dir = os.path.join(
            "C:\\", "Users\Lenovo\Desktop\exam_eti\containerized_tool\data_analysis_tool", f'{date.today()}')
    ...
    def multiple_dataset_apply_containing_cols_mysql(self):
            ...
            for key_col, columnData in value.items():
                for key_1, value_1 in dataframes_dict.items():
                    for key_col_1, colondata in value_1.items():
                        ...
                        if columnData.isin(colondata).all() == True and key_col == key_col_1 and key != key_1:
                            d = {"table_1": [key], "col_1": [
                                key_col], "table_2": [key_1], "col_2": [key_col_1]}
        ...
        writer = pd.ExcelWriter(
            f"containing_cols_{date.today()}.xlsx", engine="xlsxwriter")
    ...
    def find_value_mysql(self, value: str):
        ...
        writer = pd.ExcelWriter(
            f"find_value_{value}_{date.today()}.xlsx", engine="xlsxwriter")
    ...
    def fake_record_creator_sakila(self, sql_command='SELECT max(address_id) FROM address'):
        ...
        # Creating connection string and running sql command
        fake = Faker()
        last_address_id = self.multiple_dataset_apply_mysql_query(
            f'{sql_command}')
