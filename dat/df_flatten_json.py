...
def df_flatten_json(df: pd.DataFrame):
    ...
    byte_value_col = [df[i].name for i in df.columns if "b'" in str(df[i].iloc[0])]
    # Converted bytearrays to string
    for col_n in byte_value_col:
        df[col_n] = df[col_n].apply(lambda x: str(x))
    col_with_json_val = [df[i].name for i in df.columns if "{" in str(df[i].iloc[0]) and ":" in str(df[i].iloc[0]) and "b'" not in str(df[i].iloc[0])]
